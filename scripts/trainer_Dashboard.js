...
async function sendResponse(id, status) {
  let notification;
  if (status == "accepted") {
    notification = `${trainer.user.name} has accepted your request`;
  } else {
    notification = `Sorry! ${trainer.user.name} won't be available at the given time interval`;
  }
...
async function meet(bookingId, name) {
  ...
  document
    ...
    .addEventListener("click", async () => {
      ...
      const link = `https://flexfit.netlify.app/meeting.html?id=${room}`; // link to be changed after deployment
      const request = await fetch(`${URL}/book/meeting/create`, {
        ...
        body: JSON.stringify({ msg, trainer: trainer.user._id, link, name }),
      });
      ...
      const request2 = await fetch(`${URL}/book/${bookingId}/notifications`, {
        ...
        body: JSON.stringify({
          message: `${msg},${link},${trainer.user.name}`,
        }),
      });
    });
}
...
async function meeting() {
  showLoader2();
  const req = await fetch(`${URL}/book/${trainer.user._id}`, {
    method: "GET",
    headers: {
      "Content-type": "application/json",
      authorization: token,
...
loginTag.textContent = isUserName || Login;
