...
async function appendData() {
  const req = await fetch(`${url}/user/${trainer}`);
  const data = await req.json();
  const price = +data.user.price;
  const totalPrice = (price * time * (duration || 1)) / 30;
  const totalGST = Math.floor((totalPrice * 18) / 100);
...
submit.addEventListener("click", () => {
  ...
  Swal.fire("Your Payment is successfull", "", "success");
  setTimeout(() => {
